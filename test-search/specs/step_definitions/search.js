...
When('I write something in the search bar', () => {
    cy.get('.form-control').type('helmet');
    // change the word in type to test getting a failed or successful search result
});
...
// test Go to a products page by selecting a result from a search
Given(
    'I have made a successful search resulting in options of products',
    () => {
        ...
        cy.get('#searchpage-products-container > :nth-child(1)').should(
            'exist'
        );
    }
);
...
When('I click on a products view more button', () => {
    cy.get(':nth-child(1) > #home-card-btn > .btn').click();
});

Then('I get routed to that specific product page', () => {
    cy.url().should('include', '/Productpage');
    cy.url().should('match', /\/Productpage\//i);
    cy.get('#product-page-card').should('exist');
});
